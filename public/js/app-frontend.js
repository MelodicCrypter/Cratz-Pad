...
// Library Modules
import $ from 'jquery';
import '../../node_modules/@accursoft/jquery-caret/jquery.caret';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let selectedTexts; // this will contain all selection event
    ...
    textareaJQuery.bind('mouseup keydown', (e) => {
        // if (e.shiftKey) {
        ...
        // localStorage.setItem('storedSelectedTexts', selectedTexts);
    });

    // ===============================================================================
    // Button Menus : Click Events
    // =======> Disable Hot-Keys
    $('#btn-menu-disable').on('click', function () {
    ...
    $('#edit-bold').on('click', (e) => {
        ...
        if (selectedTexts !== undefined && selectedTexts !== '') {
            // detect if <strong> is not yet implemented
            // if (TextareaEditor.selectionIsBold(selectedTexts)) {
            // if already bold, strip all strong tags and
            // }

            // if implemented, bypass, else


            const strongedText = `<span class="bold">${selectedTexts}</span>`;

            textareaJQuery.html((index, text) => text.replace(selectedTexts, strongedText));

            const newTextarea = document.getElementById('textarea');

            Caret.setCaretPos(newTextarea, (caretPosition));
            Caret.setCaretPos(newTextarea, (caretPosition + 1));

            // TextareaEditor.reloadContent('#textarea');
            // delete all ranges
