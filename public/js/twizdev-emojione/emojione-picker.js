...
(function ($) {
    $.fn.emojionePicker = function (options) {
        let activeEl;
        const emojiContainer = 'emojionepicker';
        ...
        const createContainer = function (el) {
            $(`<div class="${emojiContainer}"><nav></nav></div>`).insertAfter(el);

            let navItems = '';
            $.each(emojiList,
                (k, v) => {
                    const emoji = emojione.shortnameToImage(v.header);
            ...
            $(document).on('click', `.${emojiContainer} section .emojione`,
                function () {
                    const caretPos = activeEl.selectionStart;
                    const textAreaTxt = $(activeEl).val();
                    let txtToAdd;
        ...
        const init = (function () {
            ...
            script.onload = function () {
                ...
                $.each($el, (i, el) => {
                    const picker = $(`<div class="emojionepicker-picker" data-index="${i}"></div>`).insertAfter(el);
                ...
                $('.emojionepicker-picker').click(
                    function () {
                        ...
                        const containerH = $(`.${emojiContainer}`).height();
                        const containerW = $(`.${emojiContainer}`).width();
                        const pickerOffset = $(this).offset();
